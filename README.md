...
**Playlist_Musica** (Tabela Associativa)
- id_playlist (INTEGER, FK)
- id_musica (INTEGER, FK)
- ordem_na_playlist (INTEGER)
- data_adicao (TIMESTAMP)
- PK: (id_playlist, id_musica)

**Musica_Genero** (Tabela Associativa)
- id_musica (INTEGER, FK)
- id_genero (INTEGER, FK)
- PK: (id_musica, id_genero)

**Musica_Artista** (Tabela Associativa para colaborações)
- id_musica (INTEGER, FK)
- id_artista (INTEGER, FK)
- tipo_participacao (ENUM: 'Principal', 'Featuring', 'Compositor', 'Produtor')
- PK: (id_musica, id_artista)

**Favoritos** (Tabela Associativa)
- id_utilizador (INTEGER, FK)
- id_musica (INTEGER, FK)
- tipo_reacao (ENUM: 'Like', 'Dislike')
- data_marcacao (TIMESTAMP)
- PK: (id_utilizador, id_musica)

**Utilizador_Artista_Seguindo** (Tabela Associativa)
- id_utilizador (INTEGER, FK)
- id_artista (INTEGER, FK)
- data_inicio_seguimento (TIMESTAMP)
- PK: (id_utilizador, id_artista)

**Jam_Session** (Entidade que faltava)
- id_sessao (INTEGER, PK)
- codigo_sessao (VARCHAR(10), UNIQUE)
- id_anfitriao (INTEGER, FK)
- nome_sessao (VARCHAR(100))
- ativa (BOOLEAN)
- data_criacao (TIMESTAMP)
- id_musica_atual (INTEGER, FK)

**Participante_Sessao** (Tabela Associativa)
- id_sessao (INTEGER, FK)
- id_utilizador (INTEGER, FK)
- data_entrada (TIMESTAMP)
- PK: (id_sessao, id_utilizador)

**Downloads_Offline** (Tabela que faltava)
- id_download (INTEGER, PK)
- id_utilizador (INTEGER, FK)
- id_musica (INTEGER, FK)
- id_dispositivo (INTEGER, FK)
...
**Utilizador:**
- O _email_ deve ser único e seguir formato válido (user@domain.com)
- A data de nascimento não pode ser futura
- A data de registo é automaticamente definida no momento da criação
...
**Playlist:**
- Não podem existir duas playlists com o mesmo nome para o mesmo utilizador
- Playlists públicas podem ser visualizadas por qualquer utilizador

**Assinatura:**
- A data de fim deve ser posterior à data de início
- Apenas um utilizador pode ter uma assinatura ativa de cada vez
- O preço deve ser positivo

**Histórico_Audição:**
- A duração ouvida não pode exceder a duração total da música
- A data/hora de reprodução não pode ser futura

**Qualidade_Audio:**
- O bitrate deve ser positivo
- O tamanho do ficheiro deve ser positivo
- Não podem existir duas versões com o mesmo nível de qualidade para a mesma música

**Relacionamentos:**
- Cada música deve pertencer a exatamente um álbum
- Cada álbum deve ter pelo menos uma música
- Cada música deve ter pelo menos um género
- Cada música deve ter pelo menos um artista principal
- Um utilizador não pode seguir o mesmo artista mais de uma vez
- Um utilizador não pode dar like e dislike na mesma música
- Uma playlist não pode conter a mesma música mais de uma vez
...
**RF01** - O sistema deve permitir o registo de novos utilizadores com _email_ e palavra-passe

**RF02** - O sistema deve autenticar utilizadores através de credenciais válidas

**RF03** - O sistema deve permitir aos utilizadores editar informações do perfil (nome, _email_, país, foto)
...
**RF19** - O sistema deve permitir aos utilizadores premium fazer _download_ de músicas para audição offline

**RF20** - O sistema deve verificar o tipo de assinatura antes de permitir _downloads_

**RF21** - O sistema deve registar informações sobre _downloads_ (música, utilizador, dispositivo, data)
...
**RF26** - O sistema deve permitir aos utilizadores configurar preferências de notificações (_email_, _push_)
...
**RNF03** - O _buffer_ de reprodução deve prevenir interrupções em conexões com velocidade mínima de 1,5 Mbps
...
**RNF22** - O design deve ser responsivo para adaptação a diferentes tamanhos de ecrã (_mobile_, _tablet_, _desktop_)
...
A plataforma de _streaming_ de música é um sistema digital abrangente que permite aos utilizadores aceder, organizar e reproduzir conteúdo musical através da Internet. Este domínio de aplicação engloba a gestão de um vasto catálogo de conteúdo áudio, perfis de utilizadores, sistemas de recomendação personalizados e funcionalidades sociais de partilha musical.

**1. Biblioteca de Músicas e Álbuns**

- **Músicas individuais** com metadados completos (título, duração, ano de lançamento, letra, género)
...
- **Estatística comparativas** (ex: "_Top_ 1% de ouvintes de um artista")
...
- `Streaming` em tempo real com _buffer_ adaptativo
...
O utilizador procura uma música específica e inicia a sua reprodução.
...
Interesses incluem dados analíticos sobre comportamento dos utilizadores, métricas de conversão (gratuito -> _premium_), ferramentas de campanhas promocionais, relatórios de crescimento e integração com redes sociais. O impacto é médio (impulsionam o crescimento do negócio).

6. Investidores e Acionistas

Financiadores e proprietários parciais da plataforma.
Interesses incluem o retorno sobre investimento, crescimento sustentável da base de utilizadores, redução de custos operacionais, vantagem competitiva no mercado e conformidade legal e regulatória. O. impacto é médio (influenciam decisões estratégicas).
